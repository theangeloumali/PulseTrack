...
class SessionManager {
  ...
  constructor() {
    ...
    if (typeof window !== "undefined") {
      console.log(
        "SessionManager: Disabled to prevent conflicts with auth store",
      );
  ...
  async checkAndRefreshSession(): Promise<boolean> {
    ...
    try {
      ...
      if (error) {
        ...
        console.warn(
          "Network error during session check, assuming session is valid",
        );
        return true;
      }
      ...
      if (
        expiresAt &&
        expiresAt.getTime() - now.getTime() < SESSION_EXPIRE_WARNING
      ) {
        ...
        if (refreshError) {
          ...
          console.warn(
            "Network error during session refresh, keeping current session",
          );
          return true;
        }
