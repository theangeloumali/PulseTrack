...
type Role = 'super_admin' | 'system_admin' | 'company_admin' | 'manager' | 'user';
...
export const canElevateRole = (currentUserRole: Role, targetRole: Role): boolean => {
  // Super admins can elevate anyone to any role
  if (currentUserRole === 'super_admin') return true;
  
  // System admins can elevate up to company_admin
  if (currentUserRole === 'system_admin' && roleHierarchy[targetRole] <= roleHierarchy.company_admin) return true;
  
  // Company admins can elevate up to manager
  if (currentUserRole === 'company_admin' && roleHierarchy[targetRole] <= roleHierarchy.manager) return true;
