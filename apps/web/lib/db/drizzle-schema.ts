...
export const projects = pgTable(
  "projects",
  {
    ...
    allow_external_activities: boolean("allow_external_activities").default(
      false,
    ),
    created_at: timestamp("created_at", { withTimezone: true })
      .defaultNow()
...
export const billingPeriods = pgTable(
  "billing_periods",
  {
    ...
    payment_received_date: timestamp("payment_received_date", {
      withTimezone: true,
    }),
...
export const billingRates = pgTable(
  ...
  (table) => ({
    ...
    effectiveFromIdx: index("billing_rates_effective_from_idx").on(
      table.effective_from,
    ),
    effectiveToIdx: index("billing_rates_effective_to_idx").on(
      table.effective_to,
    ),
  }),
).enableRLS();
...
export const companyBillingSettings = pgTable(
  ...
  (table) => ({
    // Enum constraints will be added via SQL migration
    // Performance indexes
    companyIdIdx: index("company_billing_settings_company_id_idx").on(
      table.company_id,
    ),
  }),
).enableRLS();
...
export const timeEntryBilling = pgTable(
  "time_entry_billing",
  {
    id: uuid("id").primaryKey().defaultRandom(),
  ...
  (table) => ({
    // Performance indexes
    timeEntryIdIdx: index("time_entry_billing_time_entry_id_idx").on(
      table.time_entry_id,
    ),
    billingPeriodIdIdx: index("time_entry_billing_billing_period_id_idx").on(
      table.billing_period_id,
    ),
    isBillableIdx: index("time_entry_billing_is_billable_idx").on(
      table.is_billable,
    ),
  }),
).enableRLS();
...
export const activities = pgTable(
  ...
  (table) => ({
    ...
    targetUserIdIdx: index("activities_target_user_id_idx").on(
...
export const paymentHistory = pgTable(
  ...
  (table) => ({
    // Performance indexes
    billingPeriodIdIdx: index("payment_history_billing_period_id_idx").on(
      table.billing_period_id,
    ),
