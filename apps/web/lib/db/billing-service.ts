...
export async function updateCompanyBillingSettings(
  companyId: string,
  updates: Partial<NewCompanyBillingSettings>,
) {
...
export async function getOutstandingPayments(companyId: string, filterUserId?: string) {
  ...
  if (filterUserId) {
    query = query.or(
      `notes.ilike.%Generated for user:%${filterUserId})%,created_by.eq.${filterUserId}`,
    );
  }
...
export async function generateNextBillingPeriod(
  ...
  return await generateBillingPeriodForCycle(
    supabase,
    companyId,
...
export async function getBillingCycleStats(
  companyId: string,
  year?: number,
  filterUserId?: string,
) {
...
export async function recalculateBillingPeriodAmount(
  ...
  try {
    ...
    console.log(
      `✅ Recalculated billing period ${billingPeriodId}: $${billingPeriod.payment_amount || 0} → $${totalAmount}`,
    );
    return updatedPeriod;
  } catch (error) {
