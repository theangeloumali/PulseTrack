...
import {subDays, startOfDay, endOfDay, startOfToday, startOfWeek, startOfMonth} from 'date-fns';
...
export async function getDashboardStatistics(
  ...
  const isAdmin = ['super_admin', 'system_admin', 'company_admin'].includes(userRole);
  ...
  const timeStats = await getTimeTrackingStatistics(userId, companyId, userRole);

  // Get team statistics
  const teamStats = await getTeamStatistics(companyId, userRole);

  // Get billing statistics (only for admins)
  const billingStats = isAdmin ? await getBillingStatistics(companyId) : undefined;
...
async function getProjectStatistics(userId: string, companyId: string, userRole: UserRole) {
  const isAdmin = ['super_admin', 'system_admin', 'company_admin'].includes(userRole);
...
async function getTicketStatistics(userId: string, companyId: string, userRole: UserRole) {
  const isAdmin = ['super_admin', 'system_admin', 'company_admin'].includes(userRole);
  const isManager = userRole === 'manager' || isAdmin;
...
async function getTimeTrackingStatistics(userId: string, companyId: string, userRole: UserRole) {
  const isAdmin = ['super_admin', 'system_admin', 'company_admin'].includes(userRole);
  ...
  if (isAdmin) {
    ...
    baseQuery = baseQuery.eq('user_id', userId).eq('tickets.projects.company_id', companyId);
  }
  ...
  const todayHours = todayData?.reduce((sum, entry) => sum + (entry.duration || 0), 0) || 0;
  const weekHours = weekData?.reduce((sum, entry) => sum + (entry.duration || 0), 0) || 0;
  const monthHours = monthData?.reduce((sum, entry) => sum + (entry.duration || 0), 0) || 0;
...
async function getBillingStatistics(companyId: string) {
  ...
  const pendingPayments =
    billingPeriods
      ?.filter((bp) => ['pending', 'sent', 'overdue'].includes(bp.payment_status))
      .reduce((sum, bp) => sum + (bp.payment_amount || 0), 0) || 0;
