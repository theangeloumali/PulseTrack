...
export const useAuthStore = create<AuthState>()((set, get) => ({
      ...
      if (data.user && data.session) {
        ...
        try {
          ...
          if (dbUser) {
            ...
            console.error(
              "No user profile found in database for:",
              data.user.id,
            );
    ...
    try {
      ...
      // Flow 2: Manual OTP code verification
        ...
        if (authResponse.error) {
          console.warn(
            "Failed with type 'signup', trying 'email'",
      ...
      if (
        ...
        !userMetadata.companySlug
      ) {
        const error = new Error(
          "User metadata is missing for new user creation.",
        );
  ...
  initialize: async () => {
    ...
    if (isInitializing) {
      console.log(
        "🔄 Auth Store: Initialization already in progress, waiting...",
      );
    ...
    try {
      ...
      if (sessionError) {
        ...
        if (isRefreshTokenError(sessionError)) {
          console.log(
            "🧹 Auth Store: Refresh token error detected, clearing corrupted state",
          );
          clearAuthState();
        ...
        console.warn(
          "❌ Auth Store: Non-auth error during session check, keeping partial state",
        );
      ...
      if (session?.user) {
        console.log(
          "🔄 Auth Store: Valid session found, setting session and user...",
        );
        set({
          session,
        ...
        try {
          ...
          if (dbUser) {
            ...
            console.log(
              "❌ Auth Store: No database user found for:",
              session.user.id,
            );
          ...
          console.error(
            "❌ Auth Store: Error fetching user from database:",
            dbError,
          );
          set({ user: null, isLoading: false, isInitializing: false });
        }
        ...
        const {
          ...
          if (newSession?.user) {
            ...
            try {
              ...
              if (dbUser) {
                console.log(
                  "🔄 Auth Store: User updated from auth change:",
                  dbUser.email,
                );
                set({ user: dbUser, isLoading: false });
              } else {
              ...
              console.error(
                "❌ Auth Store: Error fetching user on auth change:",
                error,
              );

              if (isRefreshTokenError(error)) {
                console.log(
                  "🧹 Auth Store: Refresh token error in auth change, clearing state",
                );
                clearAuthState();
                await supabase.auth.signOut();
                set({
