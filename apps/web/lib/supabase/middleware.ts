...
export async function updateSession(request: NextRequest) {
  ...
  try {
    ...
    const isAuthDiagnostic =
      request.nextUrl.pathname.startsWith("/auth-diagnostic");
    const isAuthCallback =
      request.nextUrl.pathname.startsWith("/auth/callback");
    const isRecoveryCallback = request.nextUrl.pathname.startsWith(
      "/auth/callback/recovery",
    );
    const isAcceptInvitation = request.nextUrl.pathname.startsWith(
    ...
    const isRootPath =
      request.nextUrl.pathname === "/" || request.nextUrl.pathname === "/pulse";
    if (isRootPath) {
      ...
      const isProxiedRequest =
        host === "zkidzdev.com" || host === "www.zkidzdev.com";

      if (user) {
        // User is authenticated, redirect to dashboard
        url.pathname = "/dashboard";
        console.log(
          "Middleware - Root path: Authenticated user, redirecting to dashboard",
        );
      } else {
    ...
    if (isApiRoute) {
      console.log(
        "Middleware - API route error, returning response without redirect",
      );
      return supabaseResponse;
    }
