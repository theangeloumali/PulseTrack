    ...
    const handleAuthCallback = async () => {
      ...
      try {
        ...
        if (type === "recovery") {
          ...
          if (code) {
            console.log(
              "Callback - Exchanging code for password reset session",
            );
            try {
              ...
              if (exchangeError) {
                console.error(
                  "Callback - Password reset code exchange error:",
                  exchangeError,
                );
              ...
              console.error(
                "Callback - Password reset code exchange failed:",
                codeError,
              );
              router.push("/forgot-password?error=reset_failed");
              return;
        ...
        if (type === "invite") {
          ...
          if (accessToken) {
            ...
            try {
              ...
              if (sessionError) {
                console.error(
                  "Callback - Error setting session:",
                  sessionError,
                );
          ...
          console.log(
            "Callback - No code/tokens found, checking existing session",
          );
          const {
          ...
          if (sessionData) {
            ...
            console.log(
              "Callback - No session found anywhere, redirecting to login",
            );
          ...
          if (code || accessToken) {
            ...
            try {
              ...
              if (!error && data.user) {
                ...
                console.log(
                  "Callback - Auth tokens found, checking if password reset flow",
                );

                // You might want to check user metadata or other indicators here
                // For now, let's redirect to dashboard but log this case
                console.log(
                  "Callback - Regular login with tokens, redirecting to dashboard",
                );
