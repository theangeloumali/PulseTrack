...
import { Activity, Database, Users, FolderOpen, FileText, AlertCircle, CheckCircle2, Loader2, RefreshCw, Bug, Eye, TestTube, ExternalLink } from 'lucide-react';
	...
	const runDiagnosticTests = async () => {
		...
		try {
			...
			results.auth = {
				...
				message: user ? `Authenticated as ${user.first_name} ${user.last_name} (${user.email})` : 'No user authenticated',
				details: {
					userId: user?.id,
			...
			if (user?.company_id) {
				...
				try {
					...
					addDetailedResult('Data Fetching - Projects with Counts', true, 'Fetching projects with ticket counts...', null);
					...
					if (fetchedProjects.length > 0) {
						...
						if (firstProject) {
							addDetailedResult('Project Tickets Fetch', true, `Fetching tickets for project: ${firstProject.name}`, null);
							try {
								...
								addDetailedResult('Ticket Count Verification', true, `Verifying ticket count for project: ${firstProject.name}`, null);
								...
								addDetailedResult('Project Tickets Fetch', false, null, error.message);
					...
					addDetailedResult('Data Fetching', false, null, error.message);
					results.dataFetching = {
						status: 'error',
					...
					try {
						...
						if (testTicketTitle.includes('Test Ticket')) {
							...
							try {
								...
								addDetailedResult('Ticket Creation', false, null, error.message);
								results.ticketCreation = {
									status: 'error',
						...
						addDetailedResult('Project Creation', false, null, error.message);
						results.projectCreation = {
							status: 'error',
				...
				addDetailedResult('Data Operations', false, null, 'No company ID available for data operations');
			...
			addDetailedResult('General Test Failure', false, null, error.message);
			results.general = {
				status: 'error',
		...
		<div className='grid gap-6 md:grid-cols-2 lg:grid-cols-3'>
			<Card>
				...
				<CardContent>
					...
					<p className='text-xs text-muted-foreground'>{user ? `${user.first_name} ${user.last_name}` : 'No user logged in'}</p>
				</CardContent>
			</Card>
			...
			<Card>
				...
				<CardContent>
					...
					<p className='text-xs text-muted-foreground'>{projectsError ? 'Error loading' : 'Total projects'}</p>
				</CardContent>
			</Card>
			...
			<Card>
				...
				<CardContent>
					...
					<p className='text-xs text-muted-foreground'>{ticketsError ? 'Error loading' : 'Total tickets'}</p>
				</CardContent>
			</Card>
					...
					<div className='grid gap-4 md:grid-cols-2'>
						<div className='space-y-2'>
							<div className='flex items-center space-x-2'>
								...
								<Badge className={getStatusColor(user ? 'success' : 'error')}>{user ? 'Online' : 'Offline'}</Badge>
							</div>
							<div className='flex items-center space-x-2'>
								{getStatusIcon(projectsError ? 'error' : 'success')}
								<span className='text-sm'>Project Data</span>
								<Badge className={getStatusColor(projectsError ? 'error' : 'success')}>{projectsError ? 'Error' : 'OK'}</Badge>
							</div>
						</div>
						<div className='space-y-2'>
							<div className='flex items-center space-x-2'>
								...
								<Badge className={getStatusColor(ticketsError ? 'error' : 'success')}>{ticketsError ? 'Error' : 'OK'}</Badge>
							</div>
							<div className='flex items-center space-x-2'>
								{getStatusIcon(projectsWithCountsError ? 'error' : 'success')}
								<span className='text-sm'>Aggregated Data</span>
								<Badge className={getStatusColor(projectsWithCountsError ? 'error' : 'success')}>{projectsWithCountsError ? 'Error' : 'OK'}</Badge>
							</div>
						</div>
					</div>
				</CardContent>
					...
					<div className='space-y-4'>
						<div className='flex items-center justify-between p-4 border rounded-lg'>
							...
							<div className='flex items-center space-x-2'>
								...
								<Badge className={getStatusColor(projectsError ? 'error' : 'success')}>{projectsLoading ? 'Loading' : projectsError ? 'Error' : `${projects.length} items`}</Badge>
							</div>
						</div>
						...
						<div className='flex items-center justify-between p-4 border rounded-lg'>
							...
							<div className='flex items-center space-x-2'>
								...
								<Badge className={getStatusColor(projectsWithCountsError ? 'error' : 'success')}>{projectsWithCountsLoading ? 'Loading' : projectsWithCountsError ? 'Error' : `${projectsWithCounts.length} items`}</Badge>
							</div>
						</div>
						...
						<div className='flex items-center justify-between p-4 border rounded-lg'>
							...
							<div className='flex items-center space-x-2'>
								...
								<Badge className={getStatusColor(ticketsError ? 'error' : 'success')}>{ticketsLoading ? 'Loading' : ticketsError ? 'Error' : `${tickets.length} items`}</Badge>
							</div>
						</div>
					</div>
				</CardContent>
		...
		<div className='space-y-6'>
			<Card>
				<CardHeader>
					...
					<CardDescription>Test core functionality end-to-end with detailed logging</CardDescription>
				</CardHeader>
				<CardContent>
					<div className='space-y-4'>
						<div className='grid gap-4 md:grid-cols-2'>
							<div>
								...
								<Input value={testProjectName} onChange={(e) => setTestProjectName(e.target.value)} placeholder='Enter project name' />
							</div>
							<div>
								<label className='text-sm font-medium'>Test Ticket Title</label>
								<Input value={testTicketTitle} onChange={(e) => setTestTicketTitle(e.target.value)} placeholder='Enter ticket title' />
							</div>
						</div>

						<div className='flex gap-4'>
							<Button onClick={runDiagnosticTests} disabled={isRunningTests || !user} className='flex-1'>
								{isRunningTests ? (
									<>
