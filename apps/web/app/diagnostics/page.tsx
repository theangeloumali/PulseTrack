...
export default function DiagnosticsPage() {
  ...
  const runDiagnosticTests = async () => {
    ...
    try {
      ...
      addDetailedResult('User Authentication', !!user, user ? {
        userId: user.id,
        email: user.email,
        companyId: user.company_id,
        role: user.role
      } : null, user ? null : 'No user authenticated');
        ...
        try {
          ...
          if (fetchedProjects.length > 0) {
            ...
            try {
              ...
              addDetailedResult('Ticket Count Verification', countMatch, {
                projectId: firstProject.id,
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 border rounded-lg">
              ...
              <div className="flex items-center space-x-2">
                ...
                <Badge className={getStatusColor(projectsError ? 'error' : 'success')}>
                  {projectsLoading ? 'Loading' : projectsError ? 'Error' : `${projects.length} items`}
                </Badge>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between p-4 border rounded-lg">
              ...
              <div className="flex items-center space-x-2">
                ...
                <Badge className={getStatusColor(projectsWithCountsError ? 'error' : 'success')}>
                  {projectsWithCountsLoading ? 'Loading' : projectsWithCountsError ? 'Error' : `${projectsWithCounts.length} items`}
                </Badge>
              </div>
            </div>
