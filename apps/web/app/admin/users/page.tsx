...
export default function SuperAdminUsersPage() {
  ...
  const handleDeactivateUser = async (userId: string, userEmail: string) => {
    if (!confirm(`Are you sure you want to deactivate ${userEmail}? This action can be reversed.`)) {
      return
    }
  ...
  const getRoleColor = (role: UserRole) => {
    switch (role) {
      ...
      case 'system_admin': return 'bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-400'
      case 'company_admin': return 'bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400'
      case 'manager': return 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400'
      case 'user': return 'bg-gray-100 text-gray-800 dark:bg-gray-800/20 dark:text-gray-400'
          ...
          <div className="bg-card rounded-lg shadow p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-foreground">
                  {users.filter(u => ['super_admin', 'system_admin', 'company_admin'].includes(u.role)).length}
                </p>
              </div>
            </div>
          </div>
