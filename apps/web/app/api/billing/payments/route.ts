  ...
  try {
    ...
    switch (action) {
      case "delete_payment_history":
        ...
        if (
          !paymentHistory ||
          paymentHistory.billing_periods.company_id !== user.company_id
        ) {
          return NextResponse.json(
      ...
      case "reset_payment_status":
        ...
        result = await resetBillingPeriodPaymentStatus(
          supabase,
          billingPeriodId,
          user.id,
        );
      ...
      case "delete_all_payment_history":
        ...
        result = await deleteAllPaymentHistory(
          supabase,
          billingPeriodId,
          user.id,
        );
      ...
      case "delete_multiple_outstanding":
        ...
        if (!billingPeriodIds) {
          return NextResponse.json(
            { error: "Billing period IDs are required" },
            { status: 400 },
          );
        }
        ...
        result = await deleteMultipleOutstandingPayments(
          supabase,
          idsArray,
          user.id,
        );
      ...
      case "delete_by_status":
        ...
        if (!statuses) {
          return NextResponse.json(
            { error: "Payment statuses are required" },
            { status: 400 },
          );
        }
