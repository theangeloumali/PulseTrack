...
export async function GET(request: NextRequest) {
    try {
        ...
        const periods = await getBillingPeriodsByCompany(user.company_id, filterUserId);
        ...
        switch (action) {
            ...
            case 'generate_for_user':
                ...
                if (!isAdmin && target_user_id !== user.id) {
                    return NextResponse.json({ error: 'You can only generate billing periods for yourself' }, { status: 403 });
                }
