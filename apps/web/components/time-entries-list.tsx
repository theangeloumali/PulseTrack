	...
	return (
		<Card className='p-4'>
			<div className='space-y-4'>
				...
				{timeEntries.length === 0 ? (
					<p className='text-gray-500 text-center py-8'>No time entries yet. Start tracking time to see entries here.</p>
				) : (
									...
									<div className='space-y-3'>
										<div className='grid grid-cols-2 gap-3'>
											<div>
												...
												<Input id='edit-hours' type='number' min='0' value={editingEntry?.hours || ''} onChange={(e: React.ChangeEvent<HTMLInputElement>) => editingEntry && setEditingEntry({ ...editingEntry, hours: e.target.value })} />
											</div>
											<div>
												...
												<Input
													...
													onChange={(e: React.ChangeEvent<HTMLInputElement>) => editingEntry && setEditingEntry({ ...editingEntry, minutes: e.target.value })}
												/>
											</div>
										</div>
										...
										<div>
											...
											<Textarea id='edit-description' value={editingEntry?.description || ''} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => editingEntry && setEditingEntry({ ...editingEntry, description: e.target.value })} rows={2} />
										</div>

										<div className='flex gap-2'>
											<Button size='sm' onClick={saveEdit} disabled={updateTimeEntryMutation.isPending}>
												Save
											</Button>
									...
									<div className='flex items-start justify-between'>
										<div className='flex-1'>
											<div className='flex items-center gap-3 mb-1'>
												...
												<span className='text-sm text-gray-500'>{formatDate(entry.start_time)}</span>
										...
										<div className='flex gap-1'>
											...
											<Button size='sm' variant='outline' onClick={() => deleteEntry(entry.id)} disabled={deleteTimeEntryMutation.isPending}>
