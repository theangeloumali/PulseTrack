        ...
        <Card className="border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-950">
          <CardHeader>
            ...
            <CardDescription className="text-red-600 dark:text-red-400">
              {overduePayments.length} payment{overduePayments.length !== 1 ? 's' : ''} past due date
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {overduePayments.slice(0, 3).map((period: BillingPeriod) => (
                <div key={period.id} className="flex items-center justify-between p-3 bg-white dark:bg-gray-800 rounded-lg">
                  <div className="flex-1">
                    <div className="font-medium">{period.name}</div>
                    <div className="text-sm text-muted-foreground">
                      Due: {period.payment_due_date ? format(new Date(period.payment_due_date), 'MMM dd, yyyy') : 'No due date'}
                    </div>
                  </div>
