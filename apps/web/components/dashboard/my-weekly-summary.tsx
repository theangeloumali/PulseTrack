...
export default function MyWeeklySummary() {
  ...
  const totalAmount = (summary || []).reduce((acc, entry) => {
    ...
    if (entry.time_entry_billing && entry.time_entry_billing.length > 0) {
      return (
        acc + parseFloat(entry.time_entry_billing[0].billable_amount || "0")
      );
    }
    // Fallback to simple calculation using user's hourly rate
    return (
      acc + (entry.duration || 0) * parseFloat(entry.user.hourly_rate || "0")
    ); // Duration is already in hours
  }, 0);
    ...
    <Card>
      ...
      <CardContent>
        ...
        <p className="text-xs text-muted-foreground">
          Total time tracked and billed this week
        </p>
        <ul className="mt-4 space-y-2">
          {summary &&
            summary.map((entry) => {
              ...
              let entryAmount;
              if (
                entry.time_entry_billing &&
                entry.time_entry_billing.length > 0
              ) {
                entryAmount = parseFloat(
                  entry.time_entry_billing[0].billable_amount || "0",
                );
              } else {
                entryAmount =
                  (entry.duration || 0) *
                  parseFloat(entry.user.hourly_rate || "0"); // Duration is already in hours
              }
