...
export function ContextualActionCenter({
  ...
  const getContextualActions = (): ActionItem[] => {
    ...
    const actions: ActionItem[] = [
      // Creation Actions
      {
        ...
        category: 'create',
        badge: !hasProjects ? {text: 'Get Started', variant: 'default'} : undefined,
      },
      {
        id: 'create-ticket',
        title: 'Add Task',
      ...
      },
      {
        ...
        icon: hasTimeToday ? <PlayCircle className="h-4 w-4" /> : <Clock className="h-4 w-4" />,
        href: '/time-tracking',
        priority: !hasTimeToday ? 'high' : 'medium',
        category: 'create',
        badge: !hasTimeToday ? {text: 'Start Now', variant: 'outline'} : undefined,
      },
      ...
      },
      {
        ...
        description: hasTeam ? 'Manage your team' : 'Collaborate with team members',
        icon: <UserPlus className="h-4 w-4" />,
        href: '/company/users',
        priority: !hasTeam && isManager ? 'high' : 'low',
        category: 'manage',
        condition: isManager,
        badge: !hasTeam ? {text: 'Collaborate', variant: 'outline'} : undefined,
      },
  ...
  const priorityActions = filteredActions.filter((action) => action.priority === 'high');
  const recommendedActions = filteredActions.filter((action) => action.priority === 'medium');
  const otherActions = filteredActions.filter((action) => action.priority === 'low');
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Contextual actions based on your current work</CardDescription>
      </CardHeader>
