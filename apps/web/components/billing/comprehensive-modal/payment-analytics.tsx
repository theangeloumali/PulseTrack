...
export function PaymentAnalytics({ 
  ...
  const analyticsData = useMemo(() => {
    ...
    const mockHistoricalPeriods = Array.from({ length: 12 }, (_, i) => ({
      id: `period-${i}`,
      name: `Period ${12 - i}`,
      start_date: format(subMonths(new Date(billingPeriod.start_date), 12 - i), 'yyyy-MM-dd'),
      end_date: format(subMonths(new Date(billingPeriod.end_date), 12 - i), 'yyyy-MM-dd'),
      total_amount: 15000 + (Math.random() - 0.5) * 5000,
      total_hours: 120 + (Math.random() - 0.5) * 40,
      payment_status: ['pending', 'sent', 'paid', 'overdue'][Math.floor(Math.random() * 4)] as any,
    ...
    // Calculate changes
    const amountChange = previousAmount > 0 ? ((currentAmount - previousAmount) / previousAmount) * 100 : 0;
    const hoursChange = previousHours > 0 ? ((currentHours - previousHours) / previousHours) * 100 : 0;
  ...
  const metricCards: MetricCard[] = [
    {
      ...
      trend: analyticsData.changes.amount > 0 ? 'up' : analyticsData.changes.amount < 0 ? 'down' : 'stable',
    ...
    },
    {
      ...
      trend: analyticsData.changes.hours > 0 ? 'up' : analyticsData.changes.hours < 0 ? 'down' : 'stable',
    ...
    },
    {
      ...
      trend: analyticsData.changes.rate > 0 ? 'up' : analyticsData.changes.rate < 0 ? 'down' : 'stable',
  ...
  const paymentStatusDistribution = useMemo(() => {
    const statuses = analyticsData.historical.reduce((acc, period) => {
  ...
  const averagePaymentTime = useMemo(() => {
    const paidPeriods = analyticsData.historical.filter(p => p.payment_status === 'paid');
    if (paidPeriods.length === 0) return 0;
    
    const totalDays = paidPeriods.reduce((sum, period) => sum + period.days_to_payment, 0);
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Monthly revenue over the selected time period</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="h-64 flex items-end justify-between gap-2 p-4">
            {analyticsData.historical.slice(-6).map((period, index) => {
              const height = (period.total_amount / Math.max(...analyticsData.historical.map(p => p.total_amount))) * 100;
              return (
                <div key={period.id} className="flex flex-col items-center gap-2 flex-1">
                  <div 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Historical payment status breakdown</CardDescription>
          </CardHeader>
            ...
            <div className="flex items-center justify-between p-3 bg-green-50 rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-sm text-muted-foreground">Days from invoice to payment</div>
                </div>
              </div>
              <div className="text-right">
                <div className="text-xl font-bold text-green-600">{averagePaymentTime} days</div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between p-3 bg-blue-50 rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-sm text-muted-foreground">Percentage of invoices paid</div>
                </div>
              </div>
              <div className="text-right">
                <div className="text-xl font-bold text-blue-600">
                  {((paymentStatusDistribution.find(s => s.status === 'paid')?.percentage || 0)).toFixed(0)}%
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between p-3 bg-purple-50 rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-sm text-muted-foreground">Hours per team member</div>
                </div>
              </div>
              <div className="text-right">
                <div className="text-xl font-bold text-purple-600">
                  {(analyticsData.current.hours / analyticsData.current.userCount).toFixed(1)}h
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between p-3 bg-orange-50 rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-sm text-muted-foreground">Revenue change vs previous</div>
                </div>
              </div>
              <div className="text-right">
                <div className={`text-xl font-bold ${
                  analyticsData.changes.amount > 0 ? 'text-green-600' : 'text-red-600'
                }`}>
                  {analyticsData.changes.amount > 0 ? '+' : ''}{analyticsData.changes.amount.toFixed(1)}%
                </div>
              </div>
            </div>
          </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>AI-powered insights to improve your billing performance</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
            {analyticsData.changes.amount < -10 && (
              <div className="flex items-start gap-3 p-3 bg-red-50 border border-red-200 rounded-lg">
                ...
                <div>
                  <div className="font-medium text-red-800">Revenue Decline Detected</div>
                  <div className="text-sm text-red-700">
                    Revenue is down {Math.abs(analyticsData.changes.amount).toFixed(1)}% compared to the previous period. 
                    Consider reviewing project scope or hourly rates.
                  </div>
                </div>
              </div>
            )}
            ...
            {averagePaymentTime > 35 && (
              <div className="flex items-start gap-3 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                ...
                <div>
                  <div className="font-medium text-yellow-800">Long Payment Cycles</div>
                  <div className="text-sm text-yellow-700">
                    Average payment time is {averagePaymentTime} days. Consider offering early payment discounts 
                    or implementing automated follow-ups.
                  </div>
                </div>
              </div>
            )}
            ...
            {analyticsData.changes.rate > 0 && (
              <div className="flex items-start gap-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                ...
                <div>
                  <div className="font-medium text-green-800">Rate Optimization Success</div>
                  <div className="text-sm text-green-700">
                    Your average hourly rate increased by {analyticsData.changes.rate.toFixed(1)}%. 
                    Great work on value optimization!
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="flex items-start gap-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              ...
              <div>
                ...
                <div className="text-sm text-blue-700">
                  Consider implementing milestone-based billing to improve cash flow and reduce payment delays.
                </div>
              </div>
            </div>
          </div>
