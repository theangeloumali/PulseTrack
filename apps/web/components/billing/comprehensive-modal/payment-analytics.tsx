...
export function PaymentAnalytics({billingPeriod, companyId}: PaymentAnalyticsProps) {
  ...
  const analyticsData = useMemo(() => {
    ...
    const hoursChange =
      previousHours > 0 ? ((currentHours - previousHours) / previousHours) * 100 : 0;
  ...
  const metricCards: MetricCard[] = [
    ...
    },
    {
      ...
      trend:
        analyticsData.changes.rate > 0 ? 'up' : analyticsData.changes.rate < 0 ? 'down' : 'stable',
        ...
        <CardContent>
          <div className="h-64 flex items-end justify-between gap-2 p-4">
            {analyticsData.historical.slice(-6).map((period, index) => {
              const height =
                (period.total_amount /
                  Math.max(...analyticsData.historical.map((p) => p.total_amount))) *
                100;
              return (
            ...
            <div className="flex items-center justify-between p-3 bg-blue-50 rounded-lg">
              ...
              <div className="text-right">
                <div className="text-xl font-bold text-blue-600">
                  {(
                    paymentStatusDistribution.find((s) => s.status === 'paid')?.percentage || 0
                  ).toFixed(0)}
                  %
                </div>
              </div>
              ...
              <div className="flex items-start gap-3 p-3 bg-red-50 border border-red-200 rounded-lg">
                ...
                <div>
                  ...
                  <div className="text-sm text-red-700">
                    Revenue is down {Math.abs(analyticsData.changes.amount).toFixed(1)}% compared to
                    the previous period. Consider reviewing project scope or hourly rates.
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="flex items-start gap-3 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                ...
                <div>
                  ...
                  <div className="text-sm text-yellow-700">
                    Average payment time is {averagePaymentTime} days. Consider offering early
                    payment discounts or implementing automated follow-ups.
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="flex items-start gap-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              ...
              <div>
                ...
                <div className="text-sm text-blue-700">
                  Consider implementing milestone-based billing to improve cash flow and reduce
                  payment delays.
                </div>
              </div>
            </div>
