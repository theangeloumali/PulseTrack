...
export function BillingOverview({ 
  ...
  const hoursPerUser = summaryStats.userCount > 0 ? summaryStats.totalHours / summaryStats.userCount : 0;
  const revenuePerUser = summaryStats.userCount > 0 ? summaryStats.totalAmount / summaryStats.userCount : 0;
  const averageHourlyRate = summaryStats.totalHours > 0 ? summaryStats.totalAmount / summaryStats.totalHours : 0;
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              ...
              <div className="text-sm text-muted-foreground">
                {format(new Date(billingPeriod.start_date), 'EEE, MMM dd')} - {format(new Date(billingPeriod.end_date), 'EEE, MMM dd, yyyy')}
              </div>
            </div>
...
function getUserBreakdown(billingReport: any) {
  ...
  const users: { [key: string]: { name: string; hours: number; amount: number; rate: number } } = {};
