        ...
        >
          <div className="bg-card border border-border shadow-xl rounded-3xl p-8 sm:p-10 flex flex-col items-center text-center">
            ...
            <p className="text-muted-foreground text-sm mb-8 leading-relaxed max-w-[250px]">
              {verificationStep === 'creating' ? "We've verified your identity. Provisioning your workspace." : 'Redirecting you to your enterprise dashboard.'}
            </p>

            <div className="w-full space-y-3">
               <div className="flex justify-between text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">
                 <span className={verificationStep === 'creating' ? 'text-primary' : ''}>Verified</span>
                 <span className={verificationStep === 'creating' ? 'text-primary' : (verificationStep === 'redirecting' ? 'text-primary' : '')}>Deploying</span>
                 <span className={verificationStep === 'redirecting' ? 'text-primary' : ''}>Routing</span>
               </div>
    ...
    <div className="min-h-screen flex items-center justify-center bg-background text-foreground py-12 px-4 sm:px-6 selection:bg-primary/20">
      <div className="max-w-md w-full space-y-8 absolute top-8 left-8 sm:left-12">
        <Link href="/" className="inline-flex items-center gap-2 group pb-2 outline-none">
          ...
          <span className="text-sm font-semibold text-muted-foreground group-hover:text-foreground transition-colors">Return to platform</span>
        </Link>
      </div>
      ...
      >
        <div className="bg-card border border-border shadow-xl rounded-[2rem] p-8 sm:p-10 relative overflow-hidden">
          ...
          <div className="text-center mb-8 relative z-10">
            ...
            <h2 className="text-2xl font-extrabold tracking-tight text-foreground mb-3">Security Checkpoint</h2>
            <p className="text-sm text-muted-foreground leading-relaxed">
              We've dispatched an <span className="font-bold text-foreground">8-digit</span> secure protocol code to <span className="font-bold text-foreground mt-1 px-2 py-1 bg-muted rounded-md border border-border inline-block">{email || 'your email'}</span>
            </p>
          </div>
          ...
          <form onSubmit={handleVerification} className="space-y-6 relative z-10 w-full">
            ...
            <div className="flex flex-col gap-3">
              <Input
                ...
                autoComplete="off"
              />
              <p className="text-[11px] font-bold tracking-wider uppercase text-muted-foreground text-center">Protocol Code (8 Digits)</p>
            </div>
            ...
            {retryCount > 0 && (
              <p className="text-xs text-center font-bold text-muted-foreground uppercase tracking-wider mt-4">Redial Attempt: {retryCount + 1} / 3</p>
            )}
          </form>

          <div className="mt-8 pt-6 border-t border-border text-center relative z-10 flex flex-col items-center">
            <span className="text-sm font-medium text-muted-foreground mb-1">Didn't retrieve the protocol?</span>
            <button
              type="button"
